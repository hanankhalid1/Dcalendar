...
const AppointmentScheduleScreen: React.FC<AppointmentScheduleScreenProps> = () => {
  const navigation = useNavigation<AppNavigationProp>();
  const route = useRoute<any>();
  const { appointmentId, mode } = route.params || {};
  const formMode = mode ?? null;
  const collaborateUser = formMode === 'collaborate_user' ? appointmentId : null;
  ...
  // Schedule state
  const [scheduleRepeatWeek, setScheduleRepeatWeek] = useState<Record<string, TimeSlot[]>>({
    Sun: [{ noData: 'Unavailable' }],
  ...
  const [availabilityScheduleDay, setAvailabilityScheduleDay] = useState<ScheduleDay[]>([]);
  ...
  const [bookingFormFieldRequired, setBookingFormFieldRequired] = useState(false);
  ...
  useEffect(() => {
    const fetchAppointmentData = async () => {
      if (appointmentId && (formMode === 'edit' || formMode === 'collaborate_user')) {
        try {
          setIsLoading(true);
          ...
          const response = await api('GET', `/appointmentDetailsById/${appointmentId}`, undefined, {
            headers: {
          ...
          if (appointmentData?.displaySlotTimes) {
            ...
            const filterLoginUser = displaySlotTimes.filter((user: any) => user?.userName === activeUser);

            if (filterLoginUser.length && (formMode === 'edit' || formMode === 'collaborate_user')) {
              const filterCollaborateUser = displaySlotTimes.filter((user: any) => user?.userName !== activeUser);
              ...
              setAppointmentDetails({
                ...
                availability_type: loginUserData?.availability_type ?? 'repeat_week',
                appointment_timezone: loginUserData?.timeZone ?? Intl.DateTimeFormat().resolvedOptions().timeZone,
                ...
                is_guest_permission: loginUserData?.is_guest_permission ?? false,
                location: loginUserData?.location ?? '',
                slotColor: loginUserData?.userSettings?.slotColor ?? loginUserData?.slotColor ?? generateLightColorWithContrast(),
              });
              ...
              if (loginUserData?.userSettings?.custom_time_slot) {
                setCustomTimeSlotValue(loginUserData.userSettings.custom_time_slot);
              }
  ...
  useEffect(() => {
    ...
    if (appointmentDetails.availability_type !== 'not_repeat' && availabilityScheduleDay.length > 0) {
      const availabilitySchedules = handleAvailabilityScheduleDay(availabilityScheduleDay, updatedSchedules);
  ...
  const handleToTimeChange = (
    ...
    if (!isValidTime(timeStr)) {
      showErrorToast('Invalid Time', 'Please enter a valid time format (e.g., 9:00 AM)');
      return;
    }
      ...
      if (daySchedule) {
        updatedSchedule[day] = daySchedule.map((slot: any, i: number) => {
          if (i === index) {
            ...
            const otherTime = type === 'start' 
              ? parseTime(slot.end) 
              : parseTime(slot.start);

            if (type === 'start' && newTime.isAfter(otherTime)) {
              showErrorToast('Invalid Time', 'Start time cannot be after end time');
              return slot;
            }

            if (type === 'end' && newTime.isBefore(otherTime)) {
              showErrorToast('Invalid Time', 'End time cannot be before start time');
              return slot;
            }
    ...
    setScheduleRepeatWeek((prev) => {
      ...
      if (type === 'add') {
        if (daySchedule[0]?.start) {
          ...
          if (lastEndTimeAddHour.isBefore(cutoffTime) && lastEndTimeAddTwoHour.isBefore(cutoffTime)) {
            updatedSchedule[day] = [
              ...daySchedule,
              {
  ...
  const handleManualSelect = (date: Date) => {
    const findIndex = manualScheduleDay.findIndex((dates: ScheduleDay) => {
      return moment(dates.date).format('YYYY-MM-DD') === moment(date).format('YYYY-MM-DD');
    });
    ...
    if (type === 'add' && emailRegex.test(value)) {
      try {
        ...
        if (!userExist || userExist === '') {
          showErrorToast('Error', "User doesn't exist in our dsuite applications");
          setSearchQuery('');
          return;
        }
  ...
  const handleAvailabilityDateChange = (date: Date) => {
    const findIndex = availabilityScheduleDay.findIndex((dates: ScheduleDay) => {
      return moment(dates.date).format('YYYY-MM-DD') === moment(date).format('YYYY-MM-DD');
    });
  ...
  const handleAvailabilityDateSchedule = (type: string, date: Date, index?: number) => {
    setAvailabilityScheduleDay((prev) => {
      let updatedSchedule = [...prev];
      const findIndex = updatedSchedule.findIndex(
        (dates) => dates.date.getTime() === date.getTime()
    ...
    if (bookingFormFieldName.trim()) {
      if (editFormData) {
        ...
        const updatedFormFields = bookingForm.map((field, index) => {
          if (field.fieldName === editFormData.fieldName && field.manualField === editFormData.manualField) {
            return {
  ...
  const handleLocationFocus = () => {
    if (appointmentDetails.location && appointmentDetails.location.length >= 2) {
      if (debouncedFetch.current) {
  ...
  const handleDateTimeSelect = (date: Date, time: string) => {
    if (currentTimeSlot) {
      if (currentTimeSlot.day) {
        ...
        if (availabilityScheduleDay.some(d => d.date.getTime() === currentTimeSlot.date!.getTime())) {
  ...
  const openTimePicker = (day?: string, date?: Date, index?: number, type?: 'start' | 'end') => {
  ...
  const getSelectedTimezoneData = () => {
    return (
      timezones.find((tz) => tz.id === appointmentDetails.appointment_timezone) || {
  ...
  const DateDisplayContainer = (
    ...
    handleTimeChange: (date: Date, time: string, index: number, type: 'start' | 'end') => void,
              ...
              {day.time.map((time: any, timeIndex: number) => (
                <View key={'time_slot' + timeIndex} style={styles.timeSlotRow}>
                  <TouchableOpacity
                    ...
                    onPress={() => openTimePicker(undefined, day.date, timeIndex, 'start')}
                  >
                    <Text style={styles.timePickerText}>{time.start}</Text>
                    <FeatherIcon name="clock" size={14} color={colors.grey400} />
                  </TouchableOpacity>
                  <Text style={styles.timeSeparator}>-</Text>
                  <TouchableOpacity
                    style={styles.timePickerButton}
                    onPress={() => openTimePicker(undefined, day.date, timeIndex, 'end')}
                  >
                  ...
                  <TouchableOpacity
                    onPress={() => handleSchedule('remove', day.date, timeIndex)}
                    style={styles.iconButton}
                  >
  ...
  const handleCollaborateSubmit = async () => {
    ...
    try {
      ...
      txHash = await blockchainService.storeCollaborateUsers(collaborate_users, activeAccount, token);
      ...
      const isSuccess = apiResponse?.data?.status === true || 
                       apiResponse?.data?.success === true || 
                       apiResponse?.status === 200 ||
                       (apiResponse?.data && !apiResponse?.data?.error && !apiResponse?.data?.message?.toLowerCase().includes('error'));

      if (isSuccess) {
        success('Success', apiResponse.data.message || 'Collaboration settings saved successfully');
        navigation.goBack();
      } else {
        const errorMessage = txHash
          ? `Server sync failed after blockchain success. Response: ${JSON.stringify(apiResponse?.data)}`
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity style={styles.closeButton} onPress={handleClose}>
          <FeatherIcon name="arrow-left" size={20} color={colors.blackText} />
        </TouchableOpacity>

        <View style={styles.headerTitleContainer}>
          <Text style={styles.headerTitle}>
            {formMode === 'edit' || formMode === 'collaborate_user' ? 'Edit ' : 'Create '}
          </Text>
          <GradientText
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {!collaborateUser && (
          <>
            <View style={styles.selectorItem}>
              ...
              <TouchableOpacity
                style={styles.selectorTextContainer}
                onPress={() => setShowDurationModal(true)}
              >
                <View style={styles.selectorTextWrapper}>
                  <Text style={styles.selectorText}>Appointment duration</Text>
        ...
        <View style={styles.selectorItem}>
          ...
          >
            <View style={styles.selectorTextWrapper}>
              <Text style={styles.selectorText}>General availability</Text>
              <Text style={styles.selectorSubtext}>
          ...
          >
            <View style={styles.modalContent}>
              <TouchableOpacity
                style={styles.modalOption}
                onPress={() => {
                  handleAvailabilityChange('repeat_week');
                  setShowAvailabilityModal(false);
                }}
              >
                <Text style={styles.modalOptionText}>Repeat weekly</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.modalOption}
                onPress={() => {
                  handleAvailabilityChange('not_repeat');
                  setShowAvailabilityModal(false);
                }}
              >
                <Text style={styles.modalOptionText}>Doesn't repeat</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={styles.modalOption}
                onPress={() => {
                  handleAvailabilityChange('custom');
                  setShowAvailabilityModal(false);
                }}
              >
                    ...
                    <View key={index} style={styles.timeSlotRow}>
                      {time.noData ? (
                        ...
                        <>
                          <TouchableOpacity
                            style={styles.timePickerButton}
                            onPress={() => openTimePicker(day, undefined, index, 'start')}
                          >
                            <Text style={styles.timePickerText}>{time.start}</Text>
                            <FeatherIcon name="clock" size={14} color={colors.grey400} />
                          </TouchableOpacity>
                          ...
                          <TouchableOpacity
                            style={styles.timePickerButton}
                            onPress={() => openTimePicker(day, undefined, index, 'end')}
                          >
                            <Text style={styles.timePickerText}>{time.end}</Text>
                            <FeatherIcon name="clock" size={14} color={colors.grey400} />
                          </TouchableOpacity>
                      ...
                      <View style={styles.timeSlotActions}>
                        {!time.noData && (
                          <>
                            <TouchableOpacity
                              onPress={() => handleAvailability(day, 'remove', index)}
                              style={styles.iconButton}
                            >
                              <FeatherIcon name="x" size={14} color={colors.grey400} />
                            </TouchableOpacity>
                            {index === 0 && (
                              <>
                                ...
                                <TouchableOpacity
                                  onPress={() => handleAvailability(day, 'copy')}
                                  style={styles.iconButton}
                                >
                                  <FeatherIcon name="copy" size={14} color={colors.grey400} />
                                </TouchableOpacity>
                              </>
                            )}
                          </>
        ...
        {appointmentDetails.availability_type === 'not_repeat' && (
          <View style={styles.manualScheduleContainer}>
            {DateDisplayContainer(manualScheduleDay, handleManualTimeChange, handleManualSchedule)}
            <TouchableOpacity
        ...
        {appointmentDetails.availability_type === 'custom' && showRecurrenceModal && (
          <Modal
            ...
            <View style={styles.modalOverlay}>
              <View style={styles.recurrenceModalContent}>
                ...
                <Text style={styles.recurrenceModalText}>
                  Custom recurrence settings will be implemented in a future update.
                </Text>
                <TouchableOpacity
        ...
        {/* Time Zone */}
        <View style={styles.selectorItem}>
          <FeatherIcon name="globe" size={20} color="#6C6C6C" />
          <TouchableOpacity
            style={styles.selectorTextContainer}
            onPress={() => setShowTimezoneModal(true)}
          >
            <View style={styles.selectorTextWrapper}>
              <Text style={styles.selectorText}>Time Zone</Text>
              <Text style={styles.selectorSubtext}>
        ...
        {appointmentDetails.availability_type !== 'not_repeat' && (
          <>
            ...
            <View style={styles.adjustedAvailabilityContainer}>
              ...
              >
                <Text style={styles.addDateButtonText}>Change a date's availability</Text>
              </TouchableOpacity>
            </View>
            <View style={styles.divider} />
          </>
                ...
                {!locationType && (
                  <View style={styles.locationTypeButtons}>
                    ...
                    >
                      ...
                      <Text style={styles.locationTypeButtonText}>In-person</Text>
                    </TouchableOpacity>
                  </View>
                )}
                ...
                {locationType && (
                  <View style={styles.selectedLocationContainer}>
                    <View style={styles.selectedLocationRow}>
                      ...
                      >
                        <FeatherIcon name="x" size={16} color={colors.grey400} />
                      </TouchableOpacity>
                    </View>
          ...
          <>
            <View style={styles.selectorItem}>
              ...
              <View style={styles.expandableSection}>
                ...
                >
                  <View style={styles.selectorTextWrapper}>
                    <Text style={styles.selectorText}>Booked appointment settings</Text>
                    <Text style={styles.selectorSubtext}>
                ...
                {bookAppointment && (
                  <View style={styles.bookingSettingsContent}>
                    ...
                    <View style={styles.settingRow}>
                      ...
                      <View style={styles.settingControls}>
                        <Switch
                          ...
                          onValueChange={(value) =>
                            setAppointmentDetails(prev => ({ ...prev, is_buffer_time: value }))
                          }
                        />
                        <TextInput
                          style={[
                            styles.numberInput,
                            !appointmentDetails.is_buffer_time && styles.disabledInput,
                          ]}
                          value={appointmentDetails.buffer_time.toString()}
                          onChangeText={(text) => {
                            const num = parseInt(text) || 0;
                            setAppointmentDetails(prev => ({ ...prev, buffer_time: num }));
                          }}
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() => {
                            ...
                            const newType = appointmentDetails.buffer_time_type === 'mm' ? 'hh' : 'mm';
                            if (newType === 'hh' && appointmentDetails.buffer_time > 12) {
                              setErrorDetails(prev => ({ ...prev, bufferTime: true }));
                            } else {
                              setErrorDetails(prev => ({ ...prev, bufferTime: false }));
                              setAppointmentDetails(prev => ({ ...prev, buffer_time_type: newType }));
                            }
                          }}
                        >
                          <Text style={styles.selectButtonText}>
                            {appointmentDetails.buffer_time_type === 'mm' ? 'Minutes' : 'Hours'}
                          </Text>
                        </TouchableOpacity>
                      </View>
                    ...
                    <View style={styles.settingRow}>
                      <View style={styles.settingLabelContainer}>
                        <Text style={styles.settingLabel}>Maximum booking per day</Text>
                        <Text style={styles.settingHint}>
                      ...
                      <View style={styles.settingControls}>
                        <Switch
                          ...
                          onValueChange={(value) =>
                            setAppointmentDetails(prev => ({ ...prev, is_maximum_per_day: value }))
                          }
                        />
                        <TextInput
                          style={[
                            styles.numberInput,
                            !appointmentDetails.is_maximum_per_day && styles.disabledInput,
                          ]}
                          value={appointmentDetails.maximum_per_day.toString()}
                          onChangeText={(text) => {
                            const num = parseInt(text) || 0;
                            setAppointmentDetails(prev => ({ ...prev, maximum_per_day: num }));
                          }}
                          keyboardType="numeric"
                    ...
                    <View style={styles.settingRow}>
                      <View style={styles.settingLabelContainer}>
                        <Text style={styles.settingLabel}>Guest Permission</Text>
                        <Text style={styles.settingHint}>
                      ...
                      <Switch
                        ...
                        onValueChange={(value) =>
                          setAppointmentDetails(prev => ({ ...prev, is_guest_permission: value }))
                        }
                      />
                    </View>
                  </View>
              ...
              <View style={styles.expandableSection}>
                ...
                >
                  <View style={styles.selectorTextWrapper}>
                    ...
                    <Text style={styles.selectorSubtext}>
                      Organize and collaborative user available time on your schedule.
                    </Text>
                  </View>
                  <FeatherIcon
                  ...
                  <View style={styles.collaborateContent}>
                    ...
                    {availableGuest.length > 0 && (
                      <View style={styles.guestList}>
                        <Text style={styles.guestListLabel}>Collaborate users</Text>
                      ...
                      {bookingForm.map((formField, index) => (
                        <View key={index} style={styles.bookingFormField}>
                          <Text style={styles.bookingFormFieldText}>
                            {formField.fieldName} {formField.isRequired ? '*' : ''}
                          </Text>
                            ...
                            <View style={styles.bookingFormFieldActions}>
                              <TouchableOpacity
                                onPress={() => {
                                  ...
                                  setBookingFormFieldRequired(formField.isRequired);
                                  setBookingFormFieldPopup(true);
                                }}
                              >
                                <FeatherIcon name="edit" size={14} color={colors.grey400} />
                              </TouchableOpacity>
                              <TouchableOpacity
                                onPress={() => handleRemoveBookingFormField(index)}
                              >
                                <FeatherIcon name="x" size={14} color={colors.grey400} />
                              </TouchableOpacity>
                            </View>
            ...
            <View style={styles.bookingFormModalBody}>
              ...
              >
                <LinearGradient
                  ...
                  style={styles.bookingFormModalSaveGradient}
                >
